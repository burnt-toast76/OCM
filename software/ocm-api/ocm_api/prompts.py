...
ZERO_ASSUMPTION_DOCTRINE = (
    ...
    "operating point to a module. This applies just as much to a connector's pinout "
    "(pin/function/wire_color), a pneumatic port (port label/thread/function -- each independently "
    "optional; a sensor's process-pressure tap has no correct function at all, so leave it out), and a "
    "comms connector (ref/type/protocol/role, e.g. an EtherCAT IN/OUT pair) as to any other field -- "
    "ask for all of them. Anything the source doesn't answer is OMITTED -- never estimated, never guessed, never copied "
